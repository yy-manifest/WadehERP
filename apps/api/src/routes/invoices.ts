...
export async function invoicesRoutes(app: FastifyInstance) {
  ...
  app.post("/invoices", async (req, reply) => {
    ...
    if (body.salesOrderId) {
      ...
      if (!so) return reply.code(400).send({ error: "bad_request", message: "salesOrderId is invalid for this tenant" });
    }
    ...
    const lines = body.lines.map((l) => {
      ...
      return { itemId: l.itemId, qty, unitPriceMinor: unit, lineTotalMinor: total };
    });

    const totalMinor = lines.reduce((acc, l) => acc + l.lineTotalMinor, 0n);
    ...
    const created = await prisma.$transaction(async (tx) => {
      ...
      await tx.auditEvent.create({
        data: {
          ...
          meta: { linesCount: inv.lines.length, totalMinor: inv.totalMinor.toString() },
        },
      });
  ...
  app.get("/invoices/:id", async (req, reply) => {
    ...
    const payments = await prisma.payment.findMany({
      ...
      select: { id: true, amountMinor: true, method: true, reference: true, note: true, createdAt: true },
    });
  ...
  app.post("/invoices/:id/payments", async (req, reply) => {
    ...
    const result = await prisma.$transaction(async (tx) => {
      // Serialize payments per (tenantId, invoiceId)
      ...
      if (nextPaid > total) {
        return { kind: "bad_request" as const, message: "payment exceeds invoice total" };
      }
